...
- 인기 검색어 조회 기능의 경우 트래픽이 많고 저장되어 있는 데이터가 많을 경우 지연시간이 길어질 수 있을 것 같습니다.<br/>이를 해결하기 위해 캐싱을 사용하여야 할 것 같다고 생각했으나 프로젝트에는 포함시키지 못했습니다.
    - 집계 방법
        - 배치 시스템에서 집계한 인기 검색어를 RDBMS 또는 NoSQL에 저장합니다.
        - 또는 NoSQL을 활용하여 실시간 집계합니다.(ex. Redis의 정렬된 셋 활용)
        - 비동기(Async or MQ)를 활용하여 블로그 조회와 검색어 집계를 분리합니다.
    - 조회
        - 캐시 된 데이터를 응답하여 대용량 트래픽에 대응합니다.
